...
class MainModel(nn.Module):
    ...
    def forward(self, x, x_lengths, prev_guesses):
        ...
        x = nn.utils.rnn.pack_padded_sequence(x.cpu(), x_lengths.cpu(), batch_first=True, enforce_sorted=False).to(
            self.device)
        output, hidden = self.rnn(
            x.to(torch.float32))  # ignore outputs, just use 2nd hidden state, since RNN is for encoding only
